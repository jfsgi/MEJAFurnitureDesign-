...
export const dresser: ComponentDef = {
  ...
  generate(p): GeneratedModel {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const boxH = Math.max(h - BOX_HEIGHT_CLEARANCE, inch(0.75));
      const boxBottomZ = frontZ - h / 2 + inch(0.25);
      const boxZ = boxBottomZ + boxH / 2;
      ...
      for (const sx of [-1, 1]) {
        parts.push({
          ...
          material: boxMat,
          primitives: [
            { shape: 'box', size: [sideT, boxD, boxH], at: [sx * (boxW / 2 - sideT / 2), boxY, boxZ] },
          ],
      ...
      for (const sy of [-1, 1]) {
        parts.push({
          ...
          material: boxMat,
          primitives: [
            { shape: 'box', size: [endW, sideT, boxH], at: [0, boxY + sy * (boxD / 2 - sideT / 2), boxZ] },
          ],
      ...
      parts.push({
        ...
        primitives: [
          {
            shape: 'box',
            size: [endW, boxD - 2 * sideT, inch(0.25)],
            at: [0, boxY, boxBottomZ + BOTTOM_RECESS + inch(0.125)],
          },
        ],
        cut: { length: endW, width: boxD - 2 * sideT, thickness: inch(0.25) },
      });
    }
